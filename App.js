// 1. Browser-friendly library definitions
const { useState, useEffect, useMemo } = React;

// 2. Icon Helper (Fixes the "Element type is invalid" error)
...
// 3. Just "function App()" (Removed export default)
function App() {
  ...
  return (
    <div className={`flex flex-col h-screen ${t.bg} transition-all duration-500 font-sans overflow-hidden`}>
      <header className="px-8 pt-10 pb-2 flex justify-between items-end">
        <div>
           <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2">
            <Icon name={greeting.icon} size={20} /> {greeting.text}
          </p>
          <h1 className={`text-3xl font-black ${t.prim} flex items-center gap-2 uppercase tracking-tight`}>
            LUNA <Icon name="Heart" fill="currentColor" size={24} />
          </h1>
        </div>
      </header>
      ...
      <main className="flex-1 overflow-hidden relative px-6 pb-32">
        {activeTab === 'tasks' && (
          <div className="h-full pt-4 overflow-y-auto space-y-5">
            <div className="bg-white/40 border border-white p-4 rounded-[24px] flex items-center gap-4">
               <div className={`${t.acc} p-2 rounded-full text-white`}><Icon name="Sparkles" size={16}/></div>
            ...
            <form onSubmit={addTask} className="flex gap-2">
              ...
              />
              <button type="submit" className={`${t.acc} p-4 rounded-2xl text-white shadow-md`}><Icon name="Plus" /></button>
            </form>
              ...
              {tasks.map((tk) => (
                <div key={tk.id} onClick={() => toggleTask(tk.id)} className={`p-5 rounded-3xl flex bg-white shadow-sm border ${t.border} items-center justify-between group cursor-pointer transition-all hover:translate-x-1`}>
                  <div className="flex items-center gap-4">
                    {tk.completed ? <Icon name="CheckCircle2" className="text-green-400" /> : <Icon name="Circle" className="text-slate-200" />}
                    <span className={`font-medium ${tk.completed ? 'line-through text-slate-300' : 'text-slate-600'}`}>{tk.text}</span>
                  </div>
                  <button onClick={(e) => {e.stopPropagation(); setTasks(tasks.filter((x)=>x.id!==tk.id))}} className="opacity-0 group-hover:opacity-100 p-1 text-slate-300 hover:text-red-400 transition-all">
        ...
        {activeTab === 'journal' && (
          <div className="h-full pt-4 space-y-4 flex flex-col overflow-y-auto">
            ...
            <div className="grid gap-3">
              ...
              {reasons.map((r, i) => (
                <div key={i} className={`bg-white/50 p-4 rounded-2xl flex items-center gap-3 border ${t.border}`}>
                  <Icon name="Heart" size={14} className={t.prim} fill="currentColor" />
        ...
        {activeTab === 'music' && (
          <div className="h-full pt-4 h-[85%]">
      ...
      {celebration.show && (
        <div onClick={() => setCelebration({ ...celebration, show: false })} className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-md">
          <div className="bg-white p-8 rounded-[40px] text-center shadow-2xl max-w-sm">
            <Icon name="Sparkles" className="mx-auto text-yellow-400 mb-4" size={40} />
      ...
      <nav className="fixed bottom-8 left-6 right-6 bg-white/90 backdrop-blur-2xl p-3 flex justify-around rounded-[32px] shadow-2xl border border-white">
        {[{id:'music', i:'Music'}, {id:'tasks', i:'CheckCircle2'}, {id:'journal', i:'PenTool'}, {id:'profile', i:'Heart'}].map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`p-4 rounded-2xl transition-all ${activeTab===tab.id ? `${t.acc} text-white shadow-lg` : `${t.nav}`}`}>
            <Icon name={tab.i} />
          </button>
        ))}
      </nav>
